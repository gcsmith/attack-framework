...
dump_result_to_file (struct attack_partial_result *result)
{
  ...
  if (current_trace_num == 0)
      ...
      if (output_binary)
	{
	  ...
	  if (1 != (bytes_read = fwrite (&header, sizeof (header), 1, result_file)))
          fprintf(stderr, "%s(%d): error writing results\n", __FUNCTION__, __LINE__);
	}
      else
	{
	  /* Text output */
  ...
  if (output_binary)
    {
      ...
      if (1 != (bytes_read = fwrite (&trace_header, sizeof (trace_header), 1, result_file)))
          fprintf(stderr, "%s(%d): error writing results\n", __FUNCTION__, __LINE__);
      if (1 != (bytes_read = fwrite (result, sizeof (struct attack_partial_result), 1, result_file)))
          fprintf(stderr, "%s(%d): error writing results\n", __FUNCTION__, __LINE__);
    }
  else
    {
