...
// -----------------------------------------------------------------------------
bool crypto_aes_hw_r0::set_message(const std::vector<uint8_t> &msg)
{
    if (msg.size() != 16) {
...
// -----------------------------------------------------------------------------
bool crypto_aes_hw_r0::set_key(const std::vector<uint8_t> &key)
{
    if (key.size() != 16) {
...
// -----------------------------------------------------------------------------
int crypto_aes_hw_r0::extract_estimate(int n)
{
    assert(n < 16);
    return m_key[n];
}
